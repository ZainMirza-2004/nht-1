...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (!tokenValidation.valid || tokenValidation.action !== 'approve') {
      return new Response(
        JSON.stringify({
          success: false,
...
<body>
  <div class="container">
    <div class="header">
      <h1>✅ Parking Permit Approved</h1>
    </div>
    <div class="content">
      <h2>Dear ${permitRequest.full_name},</h2>
      <p>Great news! Your parking permit request has been approved.</p>
      ...
      <div class="permit-details">
        ...
        <div class="detail-row">
          ...
          <span class="detail-value">${permitRequest.permit_type === 'full_day' ? 'Full Day (24 hours)' : 'Time Slot'}</span>
        </div>
        ...
        <div class="detail-row">
          ...
          <span class="detail-value">${permitId || permitRequest.id}</span>
        </div>
      </div>
      
      <p>Your vehicle is now registered and authorized for parking at the property. Please ensure your vehicle registration matches the details above.</p>
      
      <p><strong>Important:</strong> Please display this email or permit ID when requested by security or property staff.</p>
      ...
      <div class="footer">
        ...
        const emailResponse = await fetch("https://api.resend.com/emails", {
          ...
          body: JSON.stringify({
            ...
            subject: "Parking Permit Approved - NH&T Estates",
        ...
        if (emailResponse.ok && emailResult.id) {
          console.log("Approval email sent to customer:", emailResult.id);
        } else {
          console.error("Error sending approval email:", emailResult);
        }
      } catch (emailError) {
...
<head>
  ...
  <title>Parking Permit Approved</title>
  <style>
...
<body>
  <div class="container">
    ...
    <h1>Parking Permit Approved</h1>
    <p>The parking permit request has been approved and a confirmation email has been sent to the customer.</p>
    ...
    return new Response(
      JSON.stringify({
        success: false,
        message: "Failed to approve parking permit. Please try again.",
      }),
      {
        status: 500,
        headers: {
