...
Deno.serve(async (req: Request) => {
  ...
  try {
    const data: AvailabilityRequest = await req.json();
    const { serviceType, bookingDate, timeSlot, packageType, cleaningGapMinutes = 30 } = data;
    ...
    const requestedDuration = getDurationFn(packageType);
    const requestedRange = calculateBookingTimeRange(bookingDate, timeSlot, requestedDuration);
    
    // Add cleaning gap to requested range
    const requestedEndWithCleaning = new Date(
      requestedRange.end.getTime() + cleaningGapMinutes * 60 * 1000
    );
    const requestedRangeWithCleaning: TimeRange = {
      start: requestedRange.start,
      end: requestedEndWithCleaning,
    };
    ...
    // Check for overlaps with existing bookings (including cleaning gap)
    ...
    for (const booking of existingBookings || []) {
      ...
      // Add cleaning gap to existing booking range
      ...
      if (timeRangesOverlap(requestedRangeWithCleaning, bookingRangeWithCleaning)) {
        isAvailable = false;
        break;
      }
    }
